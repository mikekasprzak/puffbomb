...
// - ------------------------------------------------------------------------------------------ - //
template< class Type >
inline DataArray<Type>* new_DataArray( const size_t _Size ) {
...
// - ------------------------------------------------------------------------------------------ - //
template< class Type >
inline DataArray<Type>* new_DataArray( const size_t _Size, const Type& _InitValue ) {
	// Allocate it //
...
// - ------------------------------------------------------------------------------------------ - //
template< class Type >
inline void reallocate_DataArray( DataArray<Type>** p, const size_t _NewSize ) {
	// Allocate our new block //
...
// - ------------------------------------------------------------------------------------------ - //
template< class Type >
inline void reallocate_DataArray( DataArray<Type>** p, const size_t _NewSize, const Type& _InitValue ) {
	// Allocate our new block //
	DataArray<Type>* NewDataArray = new_DataArray<Type>( _NewSize );
...
// - ------------------------------------------------------------------------------------------ - //
template< class Type >
inline void resize_DataArray( DataArray<Type>** p, const size_t _NewSize ) {
...
// - ------------------------------------------------------------------------------------------ - //
template< class Type >
inline void resize_DataArray( DataArray<Type>** p, const size_t _NewSize, const Type& _InitValue ) {
	// A cheat.  We can resize the block without reallocating
	if ( _NewSize <= (*p)->Size ) {
...
// - ------------------------------------------------------------------------------------------ - //
template< class Type >
inline void resize2_DataArray( DataArray<Type>** p, const size_t _NewSize ) {
	// A cheat.  We can resize the block without reallocating
	if ( _NewSize <= (*p)->Size ) {
		// Set the size to the new size, and we're done //
		(*p)->Size = _NewSize;
...
// - ------------------------------------------------------------------------------------------ - //
template< class Type >
inline void resize2_DataArray( DataArray<Type>** p, const size_t _NewSize, const Type& _InitValue ) {
	// A cheat.  We can resize the block without reallocating
	if ( _NewSize <= (*p)->Size ) {
		size_t OldSize = (*p)->Size;
...
// - ------------------------------------------------------------------------------------------ - //
template< class Type >
inline void pushblockback_DataArray( DataArray<Type>** p, const size_t _Value ) {
	resize2_DataArray( p, (*p)->Size + _Value );
} 
// - ------------------------------------------------------------------------------------------ - //
template< class Type >
inline void pushblockback_DataArray( DataArray<Type>** p, const size_t _Value, const Type& _InitValue ) {
	resize2_DataArray( p, (*p)->Size + _Value, _InitValue );
} 
// - ------------------------------------------------------------------------------------------ - //
template< class Type >
inline Type popblockback_DataArray( DataArray<Type>** p, const size_t _Value ) {
	// TODO: Assert if Size == 0 //
	(*p)->Size -= _Value;
